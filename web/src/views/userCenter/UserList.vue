<template>
    <div class="userListBox">
        <div class="userListLeft">
            ...
            <div class="indexBar userNameByOrder" >
                <ul v-for="(itemGroup,index) in filterAllUserName"  >
                    <li  style="background: #d9d9d9">{{index}}</li>
                    <div >
                        <li class="test" :class="userId==item.id?'userNameLight':''  " v-for="(item,index) in itemGroup" @click="userName_click(item)">{{item.name}}</li>
                    </div>
                </ul>
            </div>
        </div>
        <div class="userMessageRight_box">
            <div class="userMessageRight" v-if="userName">
            <div v-if="addUser==false">
                <div class="userMessageRight_header">
                    <div class="userMessageRight_header_top" >
                        <div class="showUserName"  :class="{disableMask:disableMask==true}">{{userName}}</div>
                        <div class="operation">
                    ...
                    <div class="userMessageRight_header_bottom" :class="{disableMask:disableMask==true}">
                        <span class="el-icon-message">&nbsp;{{userEmail}}</span>
                        <span class="el-icon-phone">{{userPhone}}</span>
                    </div>
                </div>
                <div class="userMessageRight_bottom" :class="{disableMask:disableMask==true}">
                    <div class="user_message tab">
                        <div class="tab_header" :class="{disableMask:disableMask==true}">
                            <div>
                                 &nbsp;&nbsp;&nbsp;&nbsp;<span class="el-icon-minus" style="font-size: 14px; font-weight: 800;"></span> 部门 <span>&nbsp;&nbsp;(此用户并列属于以下部门)</span>
                            </div>
                            <div>
                                <el-button class="edit el-icon-edit" v-if="hasPermission('user/distributionOrg')"  :disabled="disabled" :class="{disableMask:disableMask==true}" @click="assignDep">编辑用户归属</el-button></div>
                                <el-dialog class="outDialog" :class="{disableMask:disableMask==true}" key="userDialog"  :berore-close="handleClose" title='编辑用户归属' v-show="assignDepDialogVisible" :visible.sync="assignDepDialogVisible" width="696px" height="421px" append-to-body  :close-on-click-modal="false" :show-close="true">
                                    <el-cascader
                                        v-model="showOrgId"
                                        ref="cascader"
                                        :options="assignDepOptions"
                                        :props="props"
                                        collapse-tags
                        ...
                        <div v-if="userOwnDep.length>0">
                            <div class="tab_body" :class="{disableMask:disableMask==true}" v-for="(item,index) in userOwnDep" :key="index" >{{item.name}} </div>
                        </div>
                    ...
                    <div class="user_message tab">
                        <div class="tab_header" :class="{disableMask:disableMask==true}">
                            ...
                            <div>
                                <el-button class="edit el-icon-edit"  v-if="hasPermission('user/distributionRole')" :disabled="disabled" :class="{disableMask:disableMask==true}"  @click="assignRoles">分配角色</el-button>
                                <el-dialog class="outDialog" :class="{disableMask:disableMask==true}" key="userDialog"  :berore-close="handleClose" title='用户角色分配' v-show="assignRolesDialogVisible" :visible.sync="assignRolesDialogVisible"  width="696px" height="421px" append-to-body  :close-on-click-modal="false" :show-close="true">
                                    <!-- 角色分配弹出框 -->
                                        <div class="emptyTipsMsg">
                                            <span style="font-size: 20px; display: inline-block; margin: 5px 16px ;" class="el-icon-info"></span>
                                            <span>用户重新登陆后，变更生效。</span>
                                        </div>
                                    <el-transfer
                                        filterable
                                        ...
                                        :titles="['可用角色', '已分配角色']"
                                        v-model="aSelectedRoleId"
                                        @change="roleEidtChange"
                                        clearable
                                        :data="data">
                                    </el-transfer>
                        ...
                        <div v-if="userOwnRole.length>0"><div class="tab_body" :class="{disableMask:disableMask==true}" v-for="(item,index) in userOwnRole" :key="index" >{{item.name}}</div></div>
...
<!--            添加用户-->
            <UserListAddAndEdit v-if="addUser" :userMessage="allUserMes" @UserEditCallBack=userEditCallBack ></UserListAddAndEdit>
        </div>
            ...
            getUserName(){
                this.$axios.leansite({
                    ...
                    if (resData.code == 0) {
                        ...
                        this.userEmail = this.allUserName[this.group[0]][0].email;//刚开始右边默认显示第一个人的信息
                        this.userName = this.allUserName[this.group[0]][0].name;
                        this.loginName = this.allUserName[this.group[0]][0].loginName;
                        this.userPhone = this.allUserName[this.group[0]][0].phone;
                        this.userId = this.allUserName[this.group[0]][0].id;
                        this.status = this.allUserName[this.group[0]][0].status;
                        this.orgId = this.allUserName[this.group[0]][0].orgId;
                        this.allUserMes = this.allUserName[this.group[0]][0];
                        this.getUserOwnRole();
                        this.getUserOwnDep();
                    } else {
                        this.$message({
            ...
            getUserOwnRole(){
                ...
                this.$axios.leansite({
                    ...
                    if (resData.code == 0) {
                        ...
                        console.log(this.aSelectedRoleId)
