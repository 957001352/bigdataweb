<template>
    <el-container class="roleList">
	  	<el-aside class="outAside" width="320px">
            ...
            <div class="tableDiv">
		  		<el-table :data="filterRoleTableData" class="table" highlight-current-row :show-header=false ref="singleTable" @row-click="handleSelectRole" :row-style="{background:'#f0f0f0'}">
	                <el-table-column  align="left">
	                    <template slot-scope="scope">
	                        <p style="font-size:14px;color: #323232;">{{scope.row.name}}</p>
	                        <p style="font-size:12px;color: #969696;">{{scope.row.note}}</p>
	                    </template>
	                </el-table-column>
	            </el-table>
            </div>
    ...
    <div class="userMessageRight_box">
	  	<el-container class="rightContainer" v-if="roleWebVisible">
	  		<RoleAddAndEdit :roleObj=oRoleAddAndEdit @RoleEditCallBack=RoleEditCallBack></RoleAddAndEdit>
	  	</el-container>
				...
				this.$axios.leansite({
					...
					if(resData.code == 0) {
						...
						if(resData.data.length > 0){
							this.oRoleObject = resData.data[0];
							this.getUserData();
