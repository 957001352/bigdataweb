		...
		<el-container class="bottomContainer">
			<el-aside :width="asideWidth">
				<div class="asideHeader">
					...
					<ul>
						<li v-for="(listStyle,index) in showListStyle" :class="listStyle.isActive?'active':''" @click="selectShowStyle(listStyle.name,index)">
			...
			<el-main>
				<el-tabs v-model="editableTabsValue" type="card"  @tab-remove="removeTab">
					<el-tab-pane v-for="(item, index) in editableTabs" :key="item.name" :label="item.title" :closable="item.isClose" :name="item.name">
	...
	export default {
		...
		methods: {
			...
			deviceListCallBack(selectDevice){
				let checkSelectDevice =  JSON.stringify(selectDevice);
				if(checkSelectDevice != '{}'){//判断是否空对象
