<template>
	<div class="industryDeviceListAddAndEidt">
					...
					<div class="formText">
						<el-form label-position="top" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="10px" class="submitForm">
							...
							<el-form-item label="描述" prop="note">
								<el-input type="textarea" v-model="ruleForm.note" placeholder="请输入描述（0~50个字符）" maxlength="50"></el-input>
							</el-form-item>
						</el-form>
					</div>
				</div>
					...
					<div class="formText">
						<el-form label-position="top" :model="ruleForm" :rules="rules" ref="ruleForm" label-width="10px" class="submitForm">
							<el-form-item label="区域" prop="orgId">
								<el-cascader ref="cascader" v-model="ruleForm.orgId" :options="departMentOptions" :props="cascaderProps" collapse-tags @change="cascaderChange" clearable>
								</el-cascader>
							</el-form-item>
						</el-form>
					</div>
		...
		<div class="bottom">
			<p>*&nbsp;必填字段</p>
			<el-row>
				<el-button type="primary" @click="saveIndustryDevice('ruleForm')">提交</el-button>
	...
	export default {
		...
		methods: {
			...
			cascaderChange(chioceNode) {
				...
				//          	if(chioceNode.length==0){
				//          		this.ruleForm.orgId = "";
				//          	}else{
				//          		this.ruleForm.orgId = chioceNode[chioceNode.length-1]*1;
				//          	}
			},
			/*
			 * 重置表单
